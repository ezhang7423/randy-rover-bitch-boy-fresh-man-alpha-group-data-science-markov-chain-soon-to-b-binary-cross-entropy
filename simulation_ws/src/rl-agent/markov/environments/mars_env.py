...
with open(os.path.join(cwd, "run.txt"), "r") as fin:
    data = fin.read().splitlines()
    version = data[0].split(" ")[1]
    episode = int(data[2].split(" ")[1])
...
CRASH_DISTANCE = (
    0.49  # Min distance to obstacle (The LIDAR is in the center of the 1M Rover)
)
...
INITIAL_DISTANCE_TO_CHECKPOINT = abs(
    math.sqrt(
        ((CHECKPOINT_X - INITIAL_POS_X) ** 2) + ((CHECKPOINT_Y - INITIAL_POS_Y) ** 2)
    )
)
...
SLEEP_BETWEEN_ACTION_AND_REWARD_CALCULATION_TIME_IN_SECOND = (
    0.3  # LIDAR Scan is 5 FPS (0.2sec).
)
...
class MarsEnv(gym.Env):
    def __init__(self):
        ...
        self.current_distance_to_checkpoint = (
            INITIAL_DISTANCE_TO_CHECKPOINT  # current distance to checkpoint
        )
        ...
        self.action_space = spaces.Box(
            low=np.array([-1, 0]), high=np.array([+1, +3]), dtype=np.float32
        )
        ...
        self.current_position_pub = rospy.Publisher(
            "/current_position", Point, queue_size=3
        )
        self.distance_travelled_pub = rospy.Publisher(
            "/distance_travelled", String, queue_size=3
        )
        # ################################################################################
        ...
        self.gazebo_model_configuration_service = rospy.ServiceProxy(
    ...
    def reset(self):
        print(
            "Total Episodic Reward=%.2f" % self.reward_in_episode,
    ...
    def rover_reset(self):
        ...
        self.gazebo_model_configuration_service(
            model_name="rover",
            urdf_param_name="rover_description",
    ...
    def set_next_state(self):
        try:
            ...
            image = image.resize(
                (TRAINING_IMAGE_WIDTH, TRAINING_IMAGE_HEIGHT), PIL.Image.ANTIALIAS
            )
    ...
    def call_reward_function(self, action):
        ...
        if (
            self.max_lin_accel_x > 0
            or self.max_lin_accel_y > 0
            or self.max_lin_accel_z > 0
        ):
            avg_imu = (
                self.max_lin_accel_x + self.max_lin_accel_y + self.max_lin_accel_y
            ) / 3
        else:
            avg_imu = 0
        ...
        if not done:
            ...
            with open(os.path.join(cwd, "run.txt"), "r+") as fout:
                ...
                fout.write("\n".join(data))
                fout.truncate()
                print("reward function writing data: ")
                print("\n".join(data))
            ...
            np.save(
                os.path.join(
                    cwd, "outputs", version, str(episode), str(self.run), "outputs.npy",
                ),
                outputs,
            )
            self.run += 1
    ...
    def callback_pose(self, data):
        ...
        new_distance_to_checkpoint.data = abs(
            math.sqrt(
                ((new_position.x - CHECKPOINT_X) ** 2)
    ...
    def send_reward_to_cloudwatch(self, reward):
        try:
            ...
            cloudwatch_client.put_metric_data(
                MetricData=[
                    {"MetricName": "Episode_Reward", "Unit": "None", "Value": reward},
                    {
